...
public class JoyStickMove : MonoBehaviour
{
    ...
    */
        ...
        if (left) { transform.Translate(Vector3.left * speed * Time.deltaTime, Space.Self); }
        if (right) { transform.Translate(Vector3.right * speed * Time.deltaTime, Space.Self); }
        if (forward) { transform.Translate(Vector3.forward * speed * Time.deltaTime, Space.Self); }
        if (reverse) { transform.Translate(Vector3.back * speed * Time.deltaTime, Space.Self); }
        if (up) { transform.Translate(Vector3.up * speed * Time.deltaTime, Space.Self); }
        if (down) { transform.Translate(Vector3.down * speed * Time.deltaTime, Space.Self); }
        if(leftRotate || rightRotate || forwardRotate || reverseRotate)
        {
            if (leftRotate)
                transform.RotateAround(target.transform.position, Vector3.up, angularVelocity * Time.deltaTime );
            if (rightRotate)
                transform.RotateAround(target.transform.position, Vector3.down, angularVelocity * Time.deltaTime);
            if (forwardRotate)
                transform.RotateAround(target.transform.position, Vector3.left, angularVelocity * Time.deltaTime);
            if (reverseRotate)
                transform.RotateAround(target.transform.position, Vector3.right, angularVelocity * Time.deltaTime);
        }
    }
